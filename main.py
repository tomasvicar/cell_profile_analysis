...
class ImageViewerDrawing(QGraphicsView):
    ...
    def setArray(self, array):
        ...
        array = array[self.parent.spinBox_channel.value(), :, :]
        self.min_max = [np.min(array), np.max(array)]
        self.parent.doubleSpinBox_contrast_min.blockSignals(True)
        self.parent.doubleSpinBox_contrast_min.setValue(self.min_max[0])
        self.parent.doubleSpinBox_contrast_min.blockSignals(False)
        self.parent.doubleSpinBox_contrast_max.blockSignals(True)
        self.parent.doubleSpinBox_contrast_max.setValue(self.min_max[1])
        self.parent.doubleSpinBox_contrast_max.blockSignals(False)
        self.array = array
        self.setImage_contrast()
    
    
    def setImage_contrast(self):
        
        
        array = utils.mat2gray(self.array.astype(np.float32), [self.parent.doubleSpinBox_contrast_min.value(), self.parent.doubleSpinBox_contrast_max.value()])
        array = (array * 255).astype(np.uint8)
        array = np.stack([array, array, array, 255 * np.ones_like(array)], axis=2)
        img = utils.arrayToQImage(array)
        self.setImage(QPixmap.fromImage(img))
...
class MainWindow(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.doubleSpinBox_contrast_max = self.findChild(QtWidgets.QDoubleSpinBox,"doubleSpinBox_contrast_max")
        self.doubleSpinBox_contrast_max.valueChanged.connect(self.contrast_changed)
        
        self.spinBox_channel = self.findChild(QtWidgets.QSpinBox,"spinBox_channel")
        self.spinBox_channel.valueChanged.connect(self.channel_changed)
    ...
    def contrast_changed(self):
        self.viewer.setImage_contrast()
    
    def channel_changed(self):
        self.viewer.setArray(self.orig_image)
    ...
    def actionOpen_clicked(self):
        ...
        fileName, _ = QFileDialog.getOpenFileName(self, "Open Image", "", "Image Files (*.tiff, *.tif );;All Files (*)", options=options)
        if fileName:
            # Set the selected image on the viewer.
            
            self.orig_image = np.max(imread(fileName), axis=0)
            self.viewer.setArray(self.orig_image)
